...
class CozyTouchClient:
    ...
    def get_jwt(self):
        if self.token is None:
            self.token = self.get_token(
                username=config.get("COZYTOUCH_USERNAME"),
                password=config.get("COZYTOUCH_PASSWORD"),
            )
    ...
    def is_available(self, deviceURL):
        ...
        r = self.session.get(
            f"https://ha110-1.overkiz.com/enduser-mobile-web/enduserAPI/setup/devices/{_deviceURL}"
        )
    ...
    def get_state(self, deviceURL, nameState):
        ...
        r = self.session.get(
            f"https://ha110-1.overkiz.com/enduser-mobile-web/enduserAPI/setup/devices/{_deviceURL}/states/{nameState}"
        )
    ...
    def check_session(self):
        r = self.session.get(
            "https://ha110-1.overkiz.com/enduser-mobile-web/enduserAPI/setup/devices"
        )
