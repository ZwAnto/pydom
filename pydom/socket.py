...
def first_handshake():
    ...
    return requests.get(f"https://{config.TYDOM_HOST}:443/mediation/client?mac={config.TYDOM_MAC[-6:]}&appli=1", headers=httpHeaders, verify=False)
...
def build_digest_headers():
    ...
    return digestAuth.build_digest_header(method='GET', url=f'https://{config.TYDOM_HOST}:443/mediation/client?mac={config.TYDOM_MAC[-6:]}&appli=1')
